...
If Row-Level Security (RLS) is enabled, urBackend automatically injects a `$match` stage at the beginning of your pipeline (or after `$geoNear` / `$search` if present) to ensure users only aggregate data they have access to.
